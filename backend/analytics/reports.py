...
def generate_risk_report(trades: list[dict[str, Any]], initial_balance: float | None = None) -> RiskReport:
    ...
    for t in trades:
        ...
        # `.get("original_signal", {})` only falls back to {} when the key is
        # missing — if it's present but explicitly None, this would return
        # None and crash the .get("metadata", {}) call below (same gotcha
        # trade_grouper.py already had to guard against for balance_before).
        original_signal = t.get("original_signal") or {}
        metadata = original_signal.get("metadata") or {}
        breakdown = metadata.get("score_breakdown") or {}
        if not breakdown:
            # metadata.score_breakdown isn't always populated by the strategy
            # layer, and — for backtests loaded from the database — it's never
            # even present at all: original_signal/metadata are deliberately
            # stripped before persistence (see runner.py's _slim_sub_trades),
            # so this branch is the ONLY path saved backtests ever take.
            # Previously this only ever produced one generic "base_structure"
            # bucket (and only when score >= 40), discarding everything else.
            # Fall back to the boolean confirmation flags that ARE reliably
            # present on the raw signal — the same ones entry_confirmations

...
class HTFFVGFlipEngine(MaxHoldExit, BaseStrategy):
    """
    ...
    def _trend_ok(self, bias: str, htf_trend: str) -> bool:
        mode = str(getattr(self.params, "htf_trend_filter", "COUNTER") or "COUNTER").upper()
        if mode == "OFF":
            return True
        want_bull = (bias == "BUY") == (mode == "WITH")
        return htf_trend == ("BULLISH" if want_bull else "BEARISH")

    def _trigger_confluences(self, candles, current_time, direction: int, displacement_atr,
                             floored: bool, first_tap: bool) -> tuple[bool, str]:
        p = self.params
        ...
        if getattr(p, "require_first_tap", False) and not first_tap:
            return False, "first_tap"
        ...
        if tf_ == "RTH":
            saved = p.session_filter_enabled
            try:
                p.session_filter_enabled = True
                if not self._is_within_session(current_time):
                    return False, "rth"
            finally:
                p.session_filter_enabled = saved
        return True, ""
            ...
            if state["status"] == "AWAIT_HTF_TAP":
                ...
                for fvg in htf_fvgs:
                    ...
                    if fvg["type"] == "BULLISH" and fvg["bottom"] <= latest["low"] <= fvg["top"]:
                        if not self._trend_ok("BUY", htf_trend):
                            continue
                    ...
                    # Bearish FVG tap -> expect bounce down (SELL bias); counter-trend only if HTF trend is BULLISH
                        ...
                        if not self._trend_ok("SELL", htf_trend):
                            continue
                        state["htf_fvg_first_tap"] = not bool(fvg.get("tapped"))
                        fvg["tapped"] = True
                        state["status"] = "AWAIT_INVERSION_FVG"
                ...
                for fvg in reversed(ltf_fvgs):
                    ...
                    if fvg_time >= state.get("tap_time", pd.Timestamp.min):
                        if state["bias"] == "BUY" and fvg["type"] == "BEARISH":
                            state["m5_fvg"] = fvg
                            state["status"] = "AWAIT_RETEST" if getattr(self.params, "require_retest", True) else "AWAIT_INVERSION_CLOSE"
            ...
            if state["status"] == "AWAIT_INVERSION_CLOSE":
                ...
                if triggered:
                    ...
                    _cf_ok, _cf_why = self._trigger_confluences(
                        candles, current_time, 1 if state["bias"] == "BUY" else -1,
                        displacement_atr, floored, bool(state.get("htf_fvg_first_tap")))

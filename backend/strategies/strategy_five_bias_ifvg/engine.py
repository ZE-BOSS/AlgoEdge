...
class BiasIFVGEngine(MaxHoldExit, BaseStrategy):
    """
    ...
    def __init__(self, config: UserConfigV2):
        ...
        # Direction of the latest active H4 FVG, whatever bias_mode is.
        self.h4_dir: dict = {}
    ...
    def _init_state(self, symbol: str):
        if symbol not in self.state:
            self.state[symbol] = {
                "bias": None,
                "key_level": None,
                "status": "AWAIT_KEY_LEVEL" if self._bias_off() else "AWAIT_BIAS",
    ...
    def _bias_off(self) -> bool:
        return str(getattr(self.params, "bias_mode", "H4") or "H4").upper() == "OFF"

    def _levels_allow(self, kind: str) -> bool:
        mode = str(getattr(self.params, "key_levels", "ALL") or "ALL").upper()
        return mode == "ALL" or mode == kind

    @staticmethod
    def _take_bias(state: dict, bias: str) -> bool:
        state["bias"] = bias
        return True
    ...
    def WINDOW_BARS(self) -> dict[str, int]:  # noqa: N802 — read by strategies.windows
        return {"M5": 3000} if getattr(self.params, "require_htf_trend", False) else {}

    def _trigger_confluences(self, symbol, candles, current_time, direction: int, displacement_atr,
                             floored: bool, confluent_levels: int) -> tuple[bool, str]:
        p = self.params
        side = str(getattr(p, "side", "both")).lower()
        if (side == "long" and direction < 0) or (side == "short" and direction > 0):
            return False, f"side: {side} only"
        if confluent_levels < int(getattr(p, "min_confluent_levels", 0) or 0):
            return False, "confluent levels"
        if (getattr(p, "min_inversion_disp_atr", 0.0) or 0.0) > 0 and (
                displacement_atr is None or displacement_atr < p.min_inversion_disp_atr):
            return False, "inversion displacement"
        if getattr(p, "require_stop_ok", False) and floored:
            return False, "stop floored"
        if getattr(p, "require_h4_aligned", False) and self.h4_dir.get(symbol) != ("BUY" if direction > 0 else "SELL"):
            return False, "h4_aligned"
        need = (getattr(p, "require_htf_trend", False), getattr(p, "require_adx_trend", False),
                getattr(p, "require_vol_high", False))
        tf_ = str(getattr(p, "time_filter", "ALL") or "ALL").upper()
        if any(need) or tf_ in ("LONDON", "NEWYORK"):
            from backend.strategies.core.confluence_flags import m5_context_flags
            fl = m5_context_flags(candles, direction, need_htf=need[0], need_adx=need[1], need_vol=need[2])
            for on, key in zip(need, ("htf_trend", "adx_trend", "vol_high")):
                if on and not fl[key]:
                    return False, key
            if tf_ == "LONDON" and not fl["london"]:
                return False, "london"
            if tf_ == "NEWYORK" and not fl["newyork"]:
                return False, "newyork"
        if tf_ == "NY_OPEN":
            ts = current_time if current_time.tzinfo else current_time.tz_localize("UTC")
            et = ts.astimezone(pytz.timezone("America/New_York"))
            if not ("09:30" <= et.strftime("%H:%M") <= "11:00"):
                return False, "ny_open"
        return True, ""
    ...
    async def on_bar(self, symbol: str, timeframe: str, candles: pd.DataFrame) -> TradeSignal | None:
        ...
        # 2. Key Levels (M15 tracker update)
            ...
            if self._bias_off() or state["bias"] is not None:
                new_extra = (self._detect_cisd_and_rejections(candles, "BUY")
                             + self._detect_cisd_and_rejections(candles, "SELL")) if self._bias_off() \
                    else self._detect_cisd_and_rejections(candles, state["bias"])
                if new_extra:
        ...
        # 3. IFVG Confirmation and Entry (M5)
            ...
            if state["status"] == "AWAIT_KEY_LEVEL":
                ...
                for fvg in (reversed(m15_fvgs) if self._levels_allow("FVG") else []):
                    if (state["bias"] == "BUY" or self._bias_off()) and fvg["type"] == "BULLISH" and fvg["bottom"] <= latest["low"] <= fvg["top"] and self._take_bias(state, "BUY"):
                        state["key_level"] = fvg
                        ...
                        break
                    elif (state["bias"] == "SELL" or self._bias_off()) and fvg["type"] == "BEARISH" and fvg["bottom"] <= latest["high"] <= fvg["top"] and self._take_bias(state, "SELL"):
                        state["key_level"] = fvg
                        state["status"] = "AWAIT_IFVG_SETUP"
                        state["manipulation_leg_start"] = current_time
                        state["approach_leg_start"] = self._approach_leg_start(candles, state["bias"])
                ...
                if state["status"] == "AWAIT_KEY_LEVEL":
                    for lvl in (reversed(state.get("extra_levels", [])) if self._levels_allow("CISD_REJ") else []):
                        if (state["bias"] == "BUY" or self._bias_off()) and lvl["type"] in ["BULLISH_CISD", "BULLISH_REJECTION"] and lvl["bottom"] <= latest["low"] <= lvl["top"] and self._take_bias(state, "BUY"):
                            state["key_level"] = lvl
                            state["status"] = "AWAIT_IFVG_SETUP"
                            state["manipulation_leg_start"] = current_time
                            state["approach_leg_start"] = self._approach_leg_start(candles, state["bias"])
                            ...
                            break
                        elif (state["bias"] == "SELL" or self._bias_off()) and lvl["type"] in ["BEARISH_CISD", "BEARISH_REJECTION"] and lvl["bottom"] <= latest["high"] <= lvl["top"] and self._take_bias(state, "SELL"):
                            state["key_level"] = lvl
                            state["status"] = "AWAIT_IFVG_SETUP"
                            state["manipulation_leg_start"] = current_time
                            state["approach_leg_start"] = self._approach_leg_start(candles, state["bias"])
            ...
            if state["status"] == "AWAIT_IFVG_CLOSE":
                ...
                if getattr(self.params, "session_filter_enabled", True) and not self.gate(
                        "session_filter", self._is_within_session(current_time)):
                    return None
                ...
                # emitted (audit §10.6).
                if getattr(self.params, "day_stop_enabled", True) and (
                        state.get("wins_today", 0) >= 1 or state.get("losses_today", 0) >= 2):
                    return None
                ...
                if triggered:
                    ...
                    _cf_ok, _cf_why = self._trigger_confluences(
                        symbol, candles, current_time, 1 if state["bias"] == "BUY" else -1,
                        displacement_atr, floored, int(state.get("confluent_level_count", 0) or 0))
                    if not self.gate("trigger_confluences", _cf_ok, _cf_why):
                        state["status"] = "AWAIT_KEY_LEVEL"
                        return None

                    # Hard day-stop rule (unconditional halves only — see
                    # _can_trade_today's docstring).
                    if getattr(self.params, "day_stop_enabled", True) and not self.gate(
                            "daily_trade_cap", self._can_trade_today(state)):
                        self.log_event(

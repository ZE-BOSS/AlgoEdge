...
class BiasIFVGParams:
    ...
    bias_mode: str = "H4"
    """"H4": direction from the latest active H4 FVG (the engine's rule). "OFF": no
    H4 bias — any tapped key level sets the direction (bullish level -> buy)."""
    key_levels: str = "ALL"
    """Which M15 key levels can arm a setup: "ALL", "FVG" (M15 gaps only) or "CISD_REJ" (CISD lines and rejection blocks only)."""
    session_filter_enabled: bool = True
    """False = trigger at any time of day (session_start/session_cutoff ignored)."""
    day_stop_enabled: bool = True
    """The spec's day-stop (stop after 1 win, after 2 losses, 2nd trade after a loss clamped)."""
    min_confluent_levels: int = 0
    """At least this many other key levels must overlap the tapped one."""
    require_h4_aligned: bool = False
    """The latest active H4 FVG must point the trade's way (useful with bias_mode OFF)."""
    time_filter: str = "ALL"
    """Trigger-bar window on top of the session: "ALL", "NY_OPEN" (09:30-11:00 ET), "LONDON" or "NEWYORK" (UTC)."""

...
```json
{
  ...
  "entry_model": {
    ...
    "required_confirmations": [
      ...
      "liquidity_sweep",
      "poi_identified",
      "ltf_choch_inside_poi",
      "ltf_bos_confirmation"
    ],
    "poi_types_accepted": ["order_block", "fvg", "supply_demand_zone"],
    "min_confluences_required": 3,
    "entry_trigger": "ltf_bos_after_choch",
    "alternate_entry_trigger": "poi_wick_tap"
  },
...
|---|---|---|---|
| HTF (bias/structure) | `4H`, `1D` | `timeframes.htf` | Determines directional bias; only trade in this direction |
| LTF (entry/confirmation) | `5m`, `15m` | `timeframes.ltf` | Used for CHOCH/BOS confirmation inside a POI |
| Execution TF | `5m` | `timeframes.execution_tf` | Timeframe the order is actually triggered on |
...
## 10. Entry Model (Signal Generation)

Sequential gate — **all required confirmations must pass** before a signal fires:

```
1. htf_bos_or_choch          → sets directional bias
2. liquidity_sweep           → external liquidity taken in bias direction
3. poi_identified            → OB and/or FVG and/or S&D zone, passing their
                                 respective validity checklists (§5–7)
4. price_reaches_poi         → wait; do not chase
5. ltf_choch_inside_poi      → structure shift on LTF within the POI
6. ltf_bos_confirmation      → clean internal BOS confirms entry direction
7. min_confluences_required  → count of (OB / FVG / S&D / Fib-OTE) hits ≥ 3
```

If `entry_trigger = "ltf_bos_after_choch"`: enter on the LTF BOS candle close.
If `entry_trigger = "poi_wick_tap"` (alternate, more aggressive): enter on first wick into POI without waiting for LTF BOS — expose this as a toggle since it trades off precision for fill rate.
...
| Parameter | Default | Description |
|---|---|---|
| `placement` | `beyond_poi` | SL sits just outside the OB/FVG/S&D zone that triggered entry |
| `buffer_pips` | 2 | Extra pip buffer beyond the zone boundary |
